...
public class ClienteNaoPositivadoE2E {
  ...
  @Test
  public void insereClienteNaoPositivado() throws Exception {
    server
    ...
    // insere um cliente nao positivado
    mockMvc
        .perform(
            post("/clientes").contentType(MediaType.APPLICATION_JSON).content(writeAsJson(cliente)))
        .andExpect(status().isCreated());

    // faz a busca pela classe social do cliente e garante que foi adicionado mais um cliente da
    // classe A
    mockMvc
